...
def export_packages(request, **kwargs):
    ...
    if request.method == 'GET' and kwargs.get('format', '') == 'csv':
        ...
        packages_manager = PackagesManager()
        required_fields = ['package_name', 'upstream_url',
                           'transplatform_url', 'release_branch_mapping']
        ...
        for package in packages:
            writer.writerow([package.package_name, package.upstream_url,
                             package.transplatform_url, package.release_branch_mapping])
        return response
    return HttpResponse(status=500)
